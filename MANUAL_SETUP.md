...
3. **ファイル名を `###` の後ろのキー名に変える**(例: `hanabi_kiss.png`)
   - **PCから**: `natsu inbox` で開くフォルダに入れる → `natsu art`
   - **スマホから**: GitHubアプリでリポジトリを開き、`images` フォルダに直接
     「Add file」→「Upload files」でアップロード。
     (`inbox` フォルダは `.gitignore` で除外されておりGitHub上には存在しないため、
     スマホから使えるのは `images` フォルダへの直接アップロードだけです)

同じキー名で上書きすれば、自動生成の絵より手動の絵が優先して使われます。
次の自動実行(2時間おき)でClaude Codeが中身を確認し、問題なければゲームに
組み込みます。
