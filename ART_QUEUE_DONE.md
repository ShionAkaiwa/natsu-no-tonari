...
`ART_QUEUE.md` のうち、すでにゲームに組み込み済み(`組込済`)になった項目を
ここに移しています。読み返す必要はありません(Claude Codeも通常は読み込みません)。
...
**2026/07/23**: 志温さんが手描き(Gemini)で用意してくれた20場面分のイラストに
差し替えたことで、以下9項目の自動生成(Pollinations)版プロンプトは不要になった。
差し替え後の確認・「消していいよ」という許可を得たうえで、試行錯誤の記録として
ここに残す(ART_QUEUE.mdからは削除済み)。

### omikuji
status: 組込済(→Gemini版に差し替え)
...
なっていることを確認して一度採用したが、その後Gemini版に差し替え)

### onbu
status: 組込済(→Gemini版に差し替え)
retries: 3
(自動生成では3回とも「おんぶ」の構図がうまく描けず要ユーザー判断としていたが、
志温さんがGeminiで手描きしてくれた版を確認したところ問題なく採用でき、解決した)
...
### fukuro
status: 組込済(→Gemini版に差し替え)
prompt: (茜が線香花火の袋を掲げて笑う、玄関先の構図。Gemini版に差し替え済みのためプロンプト詳細は割愛)
...
### bg_title(初代・茜が写っているバージョン)
status: 組込済 → 2026/07/23、志温さんの希望で茜が写らないバージョンに差し替え済み
(同じキー名`bg_title`のため自動的に置き換わった)
